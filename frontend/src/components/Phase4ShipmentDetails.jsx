...
function Phase4ShipmentDetails() {
  ...
  const generateShipmentDetails = (basicShipment) => {
    ...
    const getCarrierInfo = (cargo, origin) => {
      if (cargo.includes('Pharmaceuticals'))
        return { carrier: 'Global Pharma Logistics', vessel: 'MV MedExpress' };
      if (cargo.includes('Electronics'))
        return { carrier: 'Tech Express Lines', vessel: 'MV Digital Carrier' };
      if (cargo.includes('Textiles'))
        return { carrier: 'Fashion Logistics', vessel: 'MV Textile Express' };
      if (cargo.includes('Steel'))
        return { carrier: 'Heavy Cargo Lines', vessel: 'MV Steel Dragon' };
      if (cargo.includes('Chemicals'))
        return { carrier: 'Chemical Transport Co.', vessel: 'MV Chem Safe' };
      if (origin.includes('India'))
        return { carrier: 'India Shipping Lines', vessel: 'MV Mumbai Express' };
      if (origin.includes('China'))
        return { carrier: 'China Express Lines', vessel: 'MV Dragon Gate' };
    ...
    return {
      ...
      currentLocation: getCurrentLocation(
        basicShipment.origin,
        basicShipment.destination,
        basicShipment.status
      ),
      ...
      <div className="phase4-shipment-details error">
        <div className="modern-error-container">
          ...
          <div className="error-content">
            ...
            <p>
              We couldn't locate the shipment with ID <strong>{id}</strong>. It may have been moved
              or doesn't exist.
            </p>
      ...
      <div className="tab-content">
        ...
        {activeTab === 'route' && (
          <div className="route-tab">
            <RouteOptimizer shipment={shipment} onRouteUpdate={handleRouteUpdate} />
          </div>
        )}

        {activeTab === 'geofencing' && (
          <div className="geofencing-tab">
            <GeofenceManager shipments={[shipment]} onAlertTriggered={handleGeofenceAlert} />
          </div>
        )}

        {activeTab === 'analytics' && (
          <div className="analytics-tab">
            <AdvancedAnalytics shipments={[shipment]} selectedShipment={shipment} />
          </div>
        )}
