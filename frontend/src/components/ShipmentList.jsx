...
function ShipmentList({ shipments, setShipments, backendStatus }) {
  ...
  const filteredShipments = useMemo(() => {
    return shipments
      .filter((shipment) => {
        ...
        if (filters.search) {
          ...
          const matchesSearch =
            ...
            (shipment.customerInfo?.name &&
              shipment.customerInfo.name.toLowerCase().includes(searchLower));
        ...
        if (
          filters.cargoType &&
          !shipment.cargo.toLowerCase().includes(filters.cargoType.toLowerCase())
        ) {
          return false;
        }
  ...
  const handleBulkOperation = async (operation, updateData = {}) => {
    ...
    try {
      ...
      if (operation === 'delete') {
        setShipments(
          shipments.filter((shipment) => !selectedShipments.has(shipment._id || shipment.id))
        );
              ...
              return (
                <div key={shipmentId} className={`table-row ${isSelected ? 'selected' : ''}`}>
                  ...
                  <div className="table-cell">
                    ...
                    {shipment.priority && (
                      <span
                        className={`priority-indicator priority-${shipment.priority.toLowerCase()}`}
                      >
                        {shipment.priority}
                      </span>
                    )}
