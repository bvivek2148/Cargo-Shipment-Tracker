...
const MLModels = {
  ...
  demandForecaster: {
    ...
    calculateSeasonality: (data) => {
      ...
      return monthlyAvg.map((sum, i) => (monthlyCount[i] > 0 ? sum / monthlyCount[i] : 1));
    },
        ...
        <div className="forecast-chart">
          {result.map((period, index) => (
            <div key={index} className="forecast-period">
              ...
              <div className="period-value">{Math.round(period.prediction).toLocaleString()}</div>
              <div className="period-confidence">{period.confidence}%</div>
            </div>
          ))}
        </div>
      </div>
          ...
          {Object.entries(result.risks).map(([type, risk]) => (
            <div key={type} className="risk-item">
              ...
              <div className="risk-bar">
                <div className="risk-fill" style={{ width: `${risk.score * 100}%` }}></div>
              </div>
          ...
          {Object.entries(modelAccuracy).map(([model, accuracy]) => (
            <div key={model} className="performance-item">
              ...
              <div className="performance-bar">
                <div className="performance-fill" style={{ width: `${accuracy}%` }}></div>
              </div>
            </div>
          ))}
        </div>
