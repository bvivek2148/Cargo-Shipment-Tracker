    ...
    <div className="offline-sync">
      ...
      <div
        className={`connection-status-indicator ${
          syncStatus === 'error' ? 'error' : isOnline ? 'online' : 'offline'
      ...
      >
        <div className="connection-status-content" onClick={() => setIsMinimized(!isMinimized)}>
          ...
          {!isMinimized && (
            <div className="connection-text">
              <span className="connection-label">{isOnline ? 'Connected' : 'Offline Mode'}</span>
              <span className="connection-description">
                {isOnline ? 'Real-time sync active' : 'Changes saved locally'}
              </span>
              {lastSyncTime && (
                <span className="connection-last-sync">Last sync: {formatTime(lastSyncTime)}</span>
              )}
            </div>
          )}
        ...
        {isMinimized && pendingOperations.length > 0 && (
          <div className="pending-indicator">{pendingOperations.length}</div>
        )}
          ...
          <div className="sync-progress-mini">
            <div className="progress-ring">
              <div
                ...
                style={{ transform: `rotate(${syncProgress * 3.6}deg)` }}
              ></div>
            </div>
          </div>
        )}
      ...
      <div className={`sync-status-bar ${syncStatus}`}>
        <div className="sync-status">
          <div className={`sync-icon ${getSyncStatusColor()}`}>{getSyncStatusIcon()}</div>
        ...
        <div className="sync-actions">
          {pendingOperations.length > 0 && (
            <span className="pending-count">{pendingOperations.length} pending</span>
          )}
        ...
        <div className="pending-operations">
          ...
          <div className="operations-list">
            ...
            {pendingOperations.length > 5 && (
              <div className="more-operations">+{pendingOperations.length - 5} more operations</div>
            )}
          </div>
        </div>
      )}
          ...
          {conflictResolutions.map((conflict, index) => (
            <div key={index} className="conflict-item">
              <div className="conflict-info">
                <h5>
                  {conflict.operation.entity} - {conflict.operation.type}
                </h5>
              ...
              <div className="conflict-actions">
                ...
                <button onClick={() => resolveConflict(index, 'merge')} className="btn btn-primary">
                  Merge
                </button>
              </div>
            </div>
