  ...
  const setupRealTimeListeners = () => {
    ...
    socket.on('shipment:created', (data) => {
      ...
      const newShipment = {
        ...
        estimatedDelivery:
          data.estimatedDelivery || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    socket.on('shipment:updated', (data) => {
      ...
      setTimeout(() => {
        setShipments((prev) =>
          prev.map((s) =>
            s.trackingNumber === data.trackingNumber ? { ...s, isUpdated: false } : s
          )
        );
      }, 3000);
    });
    ...
    socket.on('shipment:delivered', (data) => {
      ...
      setTimeout(() => {
        setShipments((prev) =>
          prev.map((s) =>
            s.trackingNumber === data.trackingNumber ? { ...s, isDelivered: false } : s
          )
        );
      }, 3000);
    });
    ...
    socket.on('shipment:delayed', (data) => {
      ...
      setTimeout(() => {
        setShipments((prev) =>
          prev.map((s) =>
            s.trackingNumber === data.trackingNumber ? { ...s, isDelayed: false } : s
          )
        );
      }, 3000);
    });
          ...
          >
            <div className="shipment-header">
              ...
              <div className="shipment-meta">
                <span className="last-updated">Updated {formatTime(shipment.updatedAt)}</span>
              </div>
            </div>
